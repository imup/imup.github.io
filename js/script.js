/* =========================================================
   ...
   =========================================================
     ...
     - AI 聊天页支持消息级操作（复制/重生成/删除）与Token统计
     - 作品预览支持canvas截图；AI预览统一走首页渲染路径
     - 上传图片仅保存在内存，不写入localStorage；
       预览时从内存注入，刷新即丢；HTML保留注解
     - AI通过 [图片已上传 | 尺寸: W×H]标记判断图片可用性
   ========================================================= */
(function () {
  "use strict";
  ...
  /* 会话级图片缓存：pageId: dataUrl，刷新即丢，不写localStorage */
  var sessionImages = {};
   ...
   *  - imageDataUrl有值：直接把DataURL嵌入（用于open_preview临时预览）
   *  - imageDataUrl为空 + hasImage=true：
   *      注入注解，说明原作品含图但未嵌入；
   *      imageUrl设为null，预览时由前端内存注入真实DataURL
   *  - 两者都没有：普通作品
   */
  ...
  function executeToolCall(toolName, args) {
    try {
      ...
      if (toolName === "open_preview") {
        ...
        /* 存临时预览HTML，跳首页由 renderRunner 统一渲染；
