...
(function () {
  ...
  /* ---------------------------------------------------------
     ...
     ---------------------------------------------------------
     原理：
       - iOS Safari 键盘弹出时不改变布局视口（100vh 不变）
       - 但 visualViewport.height 会变小
       - 把 .ai-page 高度实时设为 visualViewport.height
         → 页面"缩减高度"，输入框跟随到可视区底部
       - 宽度固定为可视宽度，避免被缩放
       - 不设 transform / translateY，避免"上移"观感
       - .ai-messages 已有 overflow-y: auto，自动滚动
     --------------------------------------------------------- */
